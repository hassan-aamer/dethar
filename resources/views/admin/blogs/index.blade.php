                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.blogs') }}</h4>
                                        ...
                                        @if ($result->count())
                                            @foreach ($result as $products)
                                                <tr id="row-{{ $products->id ?? '' }}">
                                                    ...
                                                    <td><img src="{{ App\Helpers\Image::getMediaUrl($products, 'blogs') }}"
                                                            alt="products" width="100"
                                                            onclick="openImage('{{ App\Helpers\Image::getMediaUrl($products, 'blogs') }}')"
                                                    ...
                                                    <td>
                                                        {{-- @can('edit products') --}}
                                                            <a href="{{ route('admin.blogs.edit', $products->id) }}">
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            ...
                                                            <form id="deleteForm{{ $products->id }}"
                                                                action="{{ route('admin.blogs.delete', $products->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.blogs.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
