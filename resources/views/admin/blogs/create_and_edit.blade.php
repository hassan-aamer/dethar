            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <form method="POST"
                                action="{{ isset($result) && $result->id ? route('admin.blogs.update', $result->id) : route('admin.blogs.store') }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xl-6 col-sm-4">
                                        <div class="mb-3 mt-3 mt-sm-0">
                                            ...
                                            <select name="blog_category_id" data-plugin="customselect"
                                                class="form-select @error('blog_category_id') is-invalid @enderror"
                                                data-placeholder="{{ __('attributes.categories') }}">
                                                <option></option>
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}"
                                                        {{ old('blog_category_id', isset($result) && $result->blog_category_id == $category->id ? 'selected' : '') }}>
                                                        {{ $category->title ?? '' }}
                                                    </option>
                                                @endforeach
                                            </select>
