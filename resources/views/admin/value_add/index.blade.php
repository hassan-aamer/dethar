                        ...
                        <div class="page-title-box">
                            {{-- @can('create products') --}}
                                <a class="btn btn-success"
                                    href="{{ route('admin.value_add.create') }}">{{ __('attributes.create') }}</a>
                            {{-- @endcan --}}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.value_add') }}</h4>
                                        ...
                                        @if ($result->count())
                                            @foreach ($result as $products)
                                                <tr id="row-{{ $products->id ?? '' }}">
                                                    ...
                                                    <td>
                                                        {{-- @can('edit products') --}}
                                                            <a href="{{ route('admin.value_add.edit', $products->id) }}">
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            ...
                                                            <form id="deleteForm{{ $products->id }}"
                                                                action="{{ route('admin.value_add.delete', $products->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.value_add.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
