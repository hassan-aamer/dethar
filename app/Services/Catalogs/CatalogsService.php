...
class CatalogsService
{
    ...
    public function store(array $request)
    {
        try {
            ...
            $catalogs = $this->itemRepository->createItem($this->model, $request);

            if (isset($request['image']) && $request['image']) {
                $catalogs->addMediaFromRequest('image')->toMediaCollection('catalogs');
            }
    ...
    public function update(array $request, int $id)
    {
        try {
            ...
            $catalogs = $this->itemRepository->getItemById($this->model, $id);
            $this->itemRepository->updateItem($this->model, $id, $request);

            if (isset($request['image']) && $request['image']) {
                $catalogs->clearMediaCollection('catalogs');
                $catalogs->addMediaFromRequest('image')->toMediaCollection('catalogs');
            }
